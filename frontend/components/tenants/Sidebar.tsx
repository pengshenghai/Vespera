...
import { ArrowLeftFromLine, ArrowRightFromLine, Construction, Contact,  Home, HousePlus, Settings, Wallet } from "lucide-react"
    ...
    return (
        <div className="w-full bg-brand-blue h-full overflow-y-auto py-10 px-[6%] flex flex-col items-start justify-between gap-20 mb-10 font-sans  " >
            ...
            onClick={() => setCollapseNav((prev) => !prev)}  >
                {collapseNav ? (<ArrowRightFromLine size={10}/>) : (<ArrowLeftFromLine size={10} />) }
            </button>
            ...
            <ul className={`w-full ${collapseNav ? "hidden" : "flex"} flex-col items-start gap-6  `} >

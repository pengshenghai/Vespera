...
import { EncryptionService, DecryptionFailedError, EncryptionError } from './encryption.service';
...
describe('EncryptionService', () => {
    ...
    describe('encrypt', () => {
        it('should encrypt plain text successfully', async () => {
            ...
            expect(encrypted).toMatch(/^{"iv":"[^"]+","data":"[^"]+","tag":"[^"]+"}$/);
        ...
        it('should reject empty/invalid plain', async () => {
            ...
            await expect(service.encrypt(null as any)).rejects.toThrow(EncryptionError);
        });
    });
    ...
    describe('decrypt', () => {
        ...
        it('should fail invalid ciphertext', async () => {
            await expect(service.decrypt('invalid')).rejects.toThrow(DecryptionFailedError);
